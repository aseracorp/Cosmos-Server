...
go build --ldflags '-extldflags "-static"' -o build/cosmos src/*.go
if [ $? -ne 0 ]; then
    exit 1
fi
go build --ldflags '-extldflags "-static"' -o build/cosmos-launcher ./src/launcher/launcher.go ./src/launcher/update.go
if [ $? -ne 0 ]; then
    exit 1
fi
