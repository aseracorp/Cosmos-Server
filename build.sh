...
env GOARCH=arm64 go build -trimpath -ldflags "-s -w" -o build/cosmos-arm64 src/*.go
if [ $? -ne 0 ]; then
    exit 1
fi
env GOARCH=arm64 go build -trimpath -ldflags "-s -w" -o build/cosmos-launcher-arm64 ./src/launcher/launcher.go ./src/launcher/update.go
if [ $? -ne 0 ]; then
    exit 1
fi
