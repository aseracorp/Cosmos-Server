...
func NewInstallRoute(w http.ResponseWriter, req *http.Request) {
	...
	if(req.Method == "POST") {
		...
		if(request.Step == "-1") {
			// A "clean install" resets the server to its initial state. The only
			// thing that needs to go is the configuration itself: everything that
			// was derived from it (the SQLite auth.db / database.db stores, TLS
			// keys & certs, nebula state, backups) gets regenerated on the next
			// boot, and the wizard re-provisions it.
			//
			// Crucially we must NOT empty the whole config folder: since v0.23.0
			// the users store (auth.db) is opened once at process start and held
			// open. Deleting /config out from under those handles would make
			// every write made afterwards - including the admin user created a
			// few steps later - land in unlinked inodes and vanish on restart.
			// Service installs never wiped the folder, which is why they were
			// unaffected; containers now behave the same way.
			utils.Log("NewInstall: Step cleanup (removing config file)")
...
func SetupRoute(w http.ResponseWriter, req *http.Request) {
	...
	if request.ClearConfig {
		...
		// Only the config file is removed, exactly like a service install.
		// Wiping the whole /config folder in container mode would delete
		// auth.db out from under the store handle opened at startup, orphaning
		// the admin user created a few steps later (and every later write).
		// The SQLite stores, TLS keys/certs and other derived state are
		// regenerated by the setup flow itself.
		configFile := utils.GetConfigFileName()
		os.Remove(configFile)
		LoadConfig()
	}
